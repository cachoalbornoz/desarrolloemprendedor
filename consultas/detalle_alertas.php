    ...
    <tr>
        ...
        $contador = 1;
            ...
            if($_POST['tipo'] == 1){// EXPEDIENTES CON ESTADO REGULAR
                ...
                "SELECT exped.id_expediente,emp.apellido, emp.nombres, emp.cod_area, emp.telefono, emp.celular, loc.nombre
                FROM expedientes as exped, rel_expedientes_emprendedores as rel_exped, emprendedores as emp, tipo_rubro_productivos as rp,
                localidades as loc 
                WHERE exped.id_expediente = rel_exped.id_expediente and rel_exped.id_emprendedor = emp.id_emprendedor and exped.id_rubro = rp.id_rubro
                and exped.id_localidad = loc.id and rel_exped.id_responsabilidad = 1 and exped.estado = $estado and year(exped.fecha_otorgamiento) $anio and rp.tipo = 0
            ...
            if($_POST['ano'] == date('Y',time())){
                ...
                localidades as loc 
                WHERE exped.id_expediente = rel_exped.id_expediente and rel_exped.id_emprendedor = emp.id_emprendedor and exped.id_rubro = rp.id_rubro
                and exped.id_localidad = loc.id and rel_exped.id_responsabilidad = 1 and exped.estado = $estado and exped.fecha_otorgamiento <= $fecha_fin and rp.tipo = 0
                ORDER BY emp.apellido, emp.nombres asc");
            }else{
                $anio = '= '.$_POST['ano'];
        ...
        while($fila = mysqli_fetch_array($tabla_expedientes)){
            ...
            if($mostrar){
                ...
                <tr>
                    ...
                    <td><a href='../expedientes/sesion_usuario_expediente.php?id=<?php echo $fila[0] ?>' title='Ver expediente <?php echo $fila[0] ?>'><?php echo $fila[1] ?></a></td>
