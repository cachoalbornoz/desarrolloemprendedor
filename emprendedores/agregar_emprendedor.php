...
$seleccion = "SELECT id_emprendedor, cuit, apellido, nombres, id_responsabilidad FROM emprendedores WHERE cuit = $cuit";
...
if (mysqli_num_rows($registro) == 0) {
    ...
    $inserta = "INSERT INTO emprendedores (apellido, nombres, dni, cuit, direccion, fecha_nac, id_ciudad, cod_area, telefono, celular, email, id_responsabilidad, id_condicion_laboral, observaciones) 
    VALUES ('$apellido', '$nombres', '$dni', '$cuit', '$direccion', '$fecha_nacimiento', $id_ciudad, '$cod_area', '$telefono', '$celular', '$email', '$id_responsabilidad', '$id_condicion_laboral', '$observaciones')";
...
$tabla_inserta = mysqli_query(
    $con,
    "INSERT INTO rel_expedientes_emprendedores (id_expediente, id_emprendedor) VALUES ($id_expediente, $id_emprendedor)"
) or die("Revisar insercion Nuevo Emprendedor");

// BORRA EL EMPRENDEDOR POR DEFAULT	

$tabla_elimina = mysqli_query(
    $con,
    "DELETE FROM rel_expedientes_emprendedores 
    WHERE id_emprendedor = 0 AND id_expediente = $id_expediente"
) or die("Revisar Eliminación Emprendedor");
