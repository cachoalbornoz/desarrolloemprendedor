...
<div class="table-responsive">
    ...
    $tabla_expedientes = mysqli_query($con, 'SELECT edc.id_expediente, exped.nro_exp_control, exped.nro_proyecto, emp.apellido, emp.nombres, loca.nombre as localidad, emp.cod_area, emp.celular, emp.telefono, emp.email, ea.usuario,
        ...
        WHERE exped.estado = 99 AND rel_exp.id_responsabilidad = 1
...
while($fila = mysqli_fetch_array($tabla_expedientes)) {
    ...
    if($registro_ubicacion = mysqli_fetch_array($tabla_ubicacion)) {
        $ubicacion = substr($registro_ubicacion['ubicacion'], 0, 8);
    } else {
        $ubicacion = 'Sin datos';
    }
    ...
    <tr>
        <td class="text-center">
            <a href="sesion_usuario_expediente.php?id=<?php print $id_expediente; ?>&id_proyecto=<?php print $id_proyecto; ?>" title="Ver expediente">
                <?php print $fila['nro_proyecto']; ?>
            </a>                
        </td>
        <td><?php print $fila['aotorga']; ?></td>
        <td><?php print $ubicacion; ?></td>
        <td><?php print $fila['apellido']; ?></td>
        <td><?php print $fila['nombres']; ?></td>
        <td><?php print $registro_deuda[0]; ?></td>
        <td><?php print $ultimo_pago; ?></td>
        <td><?php print substr($fila['localidad'], 0, 15); ?></td>
        <td class="text-center"><?php print $fila['cod_area']; ?></td>
        <td class="text-center"><?php print $fila['celular']; ?></td>
        <td class="text-center"><?php print $fila['telefono']; ?></td>
        <td><?php print $fila['email']; ?></td>
        <td><?php print $fila['usuario']; ?></td>
        <td><?php print $fecha_eliminacion; ?></td>
    </tr>
...
?>
    ...
    <tr>
        ...
        <td>&nbsp;</td>
        <td>&nbsp;</td>
        <td><b><?php print number_format($total, 2, ',', '.'); ?></b></td>
        <td>&nbsp;</td>
        <td>&nbsp;</td>
        <td>&nbsp;</td>
        <td>&nbsp;</td>
