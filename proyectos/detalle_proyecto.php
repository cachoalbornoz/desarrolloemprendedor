...
if (isset($registro_proyectos)) {
  ...
  // ACCEDER AL PROYECTO
    ...
    $lugardesarrollo   = $registro_proyectos['lugardesarrollo'];
    if ($lugardesarrollo == 0) {
        $lugardesarrollo = 'Alquilado';
    } else {
        $lugardesarrollo = 'Propio';
    }
...
$contador       = 1;
$registro_otros = mysqli_query($con, "SELECT sol.*, loc.nombre as ciudad
FROM solicitantes sol, localidades loc, rel_proyectos_solicitantes rel
WHERE loc.id = sol.id_ciudad AND rel.id_solicitante = sol.id_solicitante AND sol.id_responsabilidad = 0 AND rel.id_proyecto = $id_proyecto");
