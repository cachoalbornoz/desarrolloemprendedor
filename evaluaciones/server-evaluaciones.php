...
while ($row = mysqli_fetch_array($query)) {
    ...
    //
    $dni   = $row['dni'];
    $tabla = mysqli_query($con, "SELECT t4.icono 
        FROM proyectos t1 
        ...
        INNER JOIN tipo_estado t4 ON t1.id_estado = t4.id_estado 
        WHERE t3.dni = $dni");
    ...
    if (isset($row['informe'])) {
        $informe = '<a href="../evaluaciones/informes/' . $row['informe'] . '">
			<i class="fa fa-folder-open" aria-hidden="true" title="Ver informe"></i>
		</a>';

        $elimina = '<a href="javascript:void(0)">
        ...
        $agrega = '<a href="../evaluaciones/Agregar_Informe.php?IdProyecto=' . $row['id_proyecto'] . '&solicitante=' . $row['solicitante'] . '">
    ...
    switch ($row['id_estado']) {
        case 24:                    // PROYECTO ENVIADO
            ...
            $agregaModifica = '<a href="../evaluaciones/Agregar_Evaluacion.php?IdProyecto=' . $row['id_proyecto'] . '&solicitante=' . $row['solicitante'] . '">
            ...
            $lee = '<a href="../impresion/imprimirProyecto.php?IdProyecto=' . $row['id_proyecto'] . '">
        ...
        case 22:                   // PROYECTO RECHAZADO
            ...
            $agregaModifica = '<a href="../evaluaciones/Modificar_Evaluacion.php?IdProyecto=' . $row['id_proyecto'] . '&solicitante=' . $row['solicitante'] . '" title="Modifica evaluacion">
        ...
        case 23:                    // PROYECTO APROBADO
            ...
            $agregaModifica = '<a href="../evaluaciones/Modificar_Evaluacion.php?IdProyecto=' . $row['id_proyecto'] . '&solicitante=' . $row['solicitante'] . '" title="Corrige evaluación">
				<i class="fas fa-edit"></i></span>
			</a>';
            $imprime = '<a href="../evaluaciones/ImprimirEvaluacion.php?IdProyecto=' . $row['id_proyecto'] . '" title="Imprime evaluación ">
				<i class="fas fa-print"></i>
			</a>';
            $lee = '<a href="../impresion/imprimirProyecto.php?IdProyecto=' . $row['id_proyecto'] . '" title="Imprime proyecto ">
				<i class="far fa-eye"></i>
			</a>';
            ...
            if (($_SESSION['tipo_usuario'] == 'a' or $_SESSION['tipo_usuario'] == 'c') and $row['id_estado'] == 23) {
                ...
                $financia = '<a href="../expedientes/su_expediente_nuevo_financiado.php?solicitante=' . $row['solicitante'] . '&IdProyecto=' . $row['id_proyecto'] . '&IdRubro=' . $row['id_rubro'] . '&rubro=' . $row['rubro'] . '&IdLocalidad=' . $row['id_localidad'] . '&localidad=' . $row['localidad'] . '" title="Financia Proyecto y pasa a Expediente ">
					Fi
				</a>';
            } else {
